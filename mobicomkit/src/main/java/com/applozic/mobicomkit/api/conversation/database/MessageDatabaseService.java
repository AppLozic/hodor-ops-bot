...
public class MessageDatabaseService {
    ...
    public void updateVideoCallMetaData(String callId, String key) {

        try {
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            final Cursor cursor = db.rawQuery("SELECT metadata," +
                    " id FROM sms WHERE message = " + "'" + callId + "'"  + " order by createdAt", null);
            cursor.moveToFirst();
            ...
            if (cursor.getCount() > 0) {
                metadata = cursor.getString(cursor.getColumnIndex(MobiComDatabaseHelper.MESSAGE_METADATA));
                messageId = cursor.getLong(cursor.getColumnIndex("id"));
            }

            if (!TextUtils.isEmpty(metadata)) {
                metaMap = (Map<String, String>)GsonUtils.getObjectFromJson(metadata, Map.class);
            }

            db.close();
            ContentValues contentVlaues  = new ContentValues();
            metaMap.put(key , String.valueOf(System.currentTimeMillis()));
            contentVlaues.put(MobiComDatabaseHelper.MESSAGE_METADATA, GsonUtils.getJsonFromObject(metaMap, Map.class));
            dbHelper.getWritableDatabase().update("sms", contentVlaues, "id=" +messageId , null);
            dbHelper.close();
    ...
    public List<Message> getMessages(Long createdAt) {
        ...
        final Cursor cursor = db.rawQuery("select m1.* from sms m1 left outer join sms m2 on (m1.createdAt < m2.createdAt"
                + " and m1.channelKey = m2.channelKey and m1.contactNumbers = m2.contactNumbers and m1.deleted = m2.deleted and"
                +"  m1.messageContentType = m2.messageContentType" + messageTypeJoinClause + " ) where m2.createdAt is null "
                +  createdAtClause
                + hiddenType
                + messageTypeClause
